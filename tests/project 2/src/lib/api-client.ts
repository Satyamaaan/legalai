  ...
  private setupInterceptors() {
    ...
    this.client.interceptors.response.use(
      ...
      async (error: AxiosError) => {
        ...
        // Handle network errors - provide more helpful messages in development
        if (!error.response) {
          if (error.code === 'ECONNABORTED') {
            throw new TimeoutError('Request timeout - please check your connection');
          }
          
          // Enhanced error message for development mode
          const networkMessage = isDevelopment 
            ? 'Network connection failed - API server may be unavailable. Running in development mode with mock data.'
