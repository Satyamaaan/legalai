...
export class UploadService {
  ...
  async confirmUpload(fileId: string): Promise<TranslationJob> {
    if (isDevelopment) {
      await delay(300);
      const mockJob = createMockJob(fileId, 'document.pdf');
      console.log(`[Mock] Created job: ${mockJob.job_id}`);
      return mockJob;
    }

    try {
      const response = await apiClient.post<TranslationJob>('/upload/confirm', {
        file_id: fileId,
      });
      return response.data;
    } catch (error) {
  ...
  async uploadFile(
    ...
    try {
      ...
      const job = await this.confirmUpload(signedUrlResponse.file_id);
  ...
  async getJobStatus(jobId: string): Promise<TranslationJob> {
    ...
    try {
      const response = await apiClient.get<TranslationJob>(`/jobs/${jobId}/status`);
      return response.data;
    } catch (error) {
...
export class TranslationService {
  async startTranslation(jobId: string): Promise<TranslationJob> {
    ...
    try {
      const response = await apiClient.post<TranslationJob>(`/jobs/${jobId}/translate`);
      return response.data;
    } catch (error) {
...
export class DownloadService {
  async getDownloadUrl(jobId: string): Promise<DownloadUrlResponse> {
    ...
    try {
      const response = await apiClient.get<DownloadUrlResponse>(`/jobs/${jobId}/download`);
      return response.data;
    } catch (error) {
